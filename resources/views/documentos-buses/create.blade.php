@extends('layouts.layoutadmin')

@section('title', 'Registrar Documento de Bus')

@section('content')

    <div class="container-fluid">

        ```
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="mb-0">
                <i class="fas fa-file-alt text-primary"></i>
                Registrar Nuevo Documento
            </h4>

            <a href="{{ route('documentos-buses.index') }}" class="btn btn-outline-primary">
                <i class="fas fa-list"></i> Gestionar
            </a>
        </div>


        <!-- ===================================================== -->
        <!-- FORMULARIO -->
        <!-- ===================================================== -->

        <div class="card shadow">

            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">
                    <i class="fas fa-file-alt"></i>
                    Información del Documento
                </h5>
            </div>

            <div class="card-body">

                <form action="{{ route('documentos-buses.store') }}"
                      method="POST"
                      enctype="multipart/form-data"
                      id="formDocumento">

                    @csrf


                    <!-- ================================================= -->
                    <!-- BUS Y TIPO -->
                    <!-- ================================================= -->

                    <div class="row">

                        <!-- Selección de Bus -->
                        <div class="col-md-6 mb-3">

                            <label for="bus_id" class="form-label">
                                <i class="fas fa-bus text-primary"></i>
                                Bus <span class="text-danger">*</span>
                            </label>

                            <select name="bus_id"
                                    id="bus_id"
                                    class="form-select @error('bus_id') is-invalid @enderror"
                                    required>

                                <option value="">Seleccione un bus</option>

                                @foreach($buses as $bus)

                                    <option value="{{ $bus->id }}"
                                        {{ old('bus_id') == $bus->id ? 'selected' : '' }}>

                                        Bus {{ $bus->numero_bus }} - Placa: {{ $bus->placa }}

                                    </option>

                                @endforeach

                            </select>

                            @error('bus_id')
                            <div class="invalid-feedback d-block">
                                {{ $message }}
                            </div>
                            @enderror

                            <div id="error_bus_id"></div>

                        </div>


                        <!-- Tipo de Documento -->
                        <div class="col-md-6 mb-3">

                            <label for="tipo_documento" class="form-label">
                                <i class="fas fa-file-contract text-primary"></i>
                                Tipo de Documento <span class="text-danger">*</span>
                            </label>

                            <select name="tipo_documento"
                                    id="tipo_documento"
                                    class="form-select @error('tipo_documento') is-invalid @enderror"
                                    required>

                                <option value="">Seleccione tipo</option>

                                @foreach($tiposDocumento as $key => $tipo)

                                    <option value="{{ $key }}"
                                        {{ old('tipo_documento') == $key ? 'selected' : '' }}>

                                        {{ $tipo }}

                                    </option>

                                @endforeach

                            </select>

                            @error('tipo_documento')
                            <div class="invalid-feedback d-block">
                                {{ $message }}
                            </div>
                            @enderror

                            <div id="error_tipo_documento"></div>

                        </div>

                    </div>


                    <!-- ================================================= -->
                    <!-- NUMERO Y ARCHIVO -->
                    <!-- ================================================= -->

                    <div class="row">

                        <!-- Número de Documento -->
                        <div class="col-md-6 mb-3">

                            <label for="numero_documento" class="form-label">
                                <i class="fas fa-hashtag text-primary"></i>
                                Número de Documento <span class="text-danger">*</span>
                            </label>

                            <input type="text"
                                   name="numero_documento"
                                   id="numero_documento"
                                   class="form-control @error('numero_documento') is-invalid @enderror"
                                   value="{{ old('numero_documento') }}"
                                   placeholder="Ej: ABC-12345-2024"
                                   required>

                            @error('numero_documento')
                            <div class="invalid-feedback d-block">
                                {{ $message }}
                            </div>
                            @enderror

                            <div id="error_numero_documento"></div>

                        </div>


                        <!-- Archivo -->
                        <div class="col-md-6 mb-3">

                            <label for="archivo" class="form-label">
                                <i class="fas fa-upload text-primary"></i>
                                Archivo Digital (Opcional)
                            </label>

                            <input type="file"
                                   name="archivo"
                                   id="archivo"
                                   class="form-control @error('archivo') is-invalid @enderror"
                                   accept=".pdf,.jpg,.jpeg,.png">

                            <small class="text-muted">
                                Formatos: PDF, JPG, PNG. Máximo 5MB
                            </small>

                            @error('archivo')
                            <div class="invalid-feedback d-block">
                                {{ $message }}
                            </div>
                            @enderror

                            <div id="preview" class="mt-2"></div>

                        </div>

                    </div>


                    <!-- ================================================= -->
                    <!-- FECHAS -->
                    <!-- ================================================= -->

                    <div class="row">

                        <!-- Fecha de Emisión -->
                        <div class="col-md-6 mb-3">

                            <label for="fecha_emision" class="form-label">
                                <i class="fas fa-calendar-plus text-primary"></i>
                                Fecha de Emisión <span class="text-danger">*</span>
                            </label>

                            <input type="date"
                                   name="fecha_emision"
                                   id="fecha_emision"
                                   class="form-control @error('fecha_emision') is-invalid @enderror"
                                   value="{{ old('fecha_emision') }}"
                                   max="{{ date('Y-m-d') }}"
                                   required>

                            @error('fecha_emision')
                            <div class="invalid-feedback d-block">
                                {{ $message }}
                            </div>
                            @enderror

                            <div id="error_fecha_emision"></div>

                        </div>


                        <!-- Fecha de Vencimiento -->
                        <div class="col-md-6 mb-3">

                            <label for="fecha_vencimiento" class="form-label">
                                <i class="fas fa-calendar-times text-primary"></i>
                                Fecha de Vencimiento <span class="text-danger">*</span>
                            </label>

                            <input type="date"
                                   name="fecha_vencimiento"
                                   id="fecha_vencimiento"
                                   class="form-control @error('fecha_vencimiento') is-invalid @enderror"
                                   value="{{ old('fecha_vencimiento') }}"
                                   required>

                            @error('fecha_vencimiento')
                            <div class="invalid-feedback d-block">
                                {{ $message }}
                            </div>
                            @enderror

                            <!-- ERROR JAVASCRIPT -->
                            <div id="error_fecha_vencimiento"></div>

                            <div id="dias_vigencia" class="mt-2"></div>

                        </div>

                    </div>


                    <!-- ================================================= -->
                    <!-- OBSERVACIONES -->
                    <!-- ================================================= -->

                    <div class="mb-3">

                        <label for="observaciones" class="form-label">
                            <i class="fas fa-comment-alt text-primary"></i>
                            Observaciones (Opcional)
                        </label>

                        <textarea name="observaciones"
                                  id="observaciones"
                                  rows="3"
                                  class="form-control @error('observaciones') is-invalid @enderror"
                                  placeholder="Notas adicionales sobre el documento...">{{ old('observaciones') }}</textarea>

                        @error('observaciones')
                        <div class="invalid-feedback d-block">
                            {{ $message }}
                        </div>
                        @enderror

                    </div>


                    <!-- ================================================= -->
                    <!-- ALERTA DE VALIDACIÓN -->
                    <!-- ================================================= -->

                    <div id="alertaValidacion"
                         class="alert alert-info d-none">

                        <i class="fas fa-info-circle"></i>

                        <span id="mensajeValidacion"></span>

                    </div>


                    <!-- ================================================= -->
                    <!-- BOTONES -->
                    <!-- ================================================= -->

                    <div class="d-flex justify-content-between">

                        <a href="{{ route('documentos-buses.index') }}"
                           class="btn btn-secondary">

                            <i class="fas fa-times"></i>
                            Cancelar

                        </a>

                        <button type="submit"
                                class="btn btn-primary"
                                id="btnRegistrarDocumento">

                            <i class="fas fa-save"></i>
                            Registrar Documento

                        </button>

                    </div>

                </form>

            </div>

        </div>


        <!-- ===================================================== -->
        <!-- INFORMACIÓN ADICIONAL -->
        <!-- ===================================================== -->

        <div class="card shadow mt-4">

            <div class="card-header bg-info text-white">

                <h6 class="mb-0">
                    <i class="fas fa-lightbulb"></i>
                    Información Importante
                </h6>

            </div>

            <div class="card-body">

                <ul class="mb-0">

                    <li>
                        Los documentos con <strong>menos de 30 días</strong>
                        para vencer mostrarán alerta amarilla.
                    </li>

                    <li>
                        Los documentos vencidos mostrarán alerta roja.
                    </li>

                    <li>
                        El sistema enviará notificaciones automáticas
                        antes del vencimiento.
                    </li>

                    <li>
                        Es recomendable subir el archivo digital del documento
                        para consultas rápidas.
                    </li>

                </ul>

            </div>

        </div>
        ```

    </div>

    <!-- ===================================================== -->

    <!-- JAVASCRIPT -->

    <!-- ===================================================== -->

    <script>

        document.addEventListener('DOMContentLoaded', function () {

            const fechaEmision =
                document.getElementById('fecha_emision');

            const fechaVencimiento =
                document.getElementById('fecha_vencimiento');

            const diasVigencia =
                document.getElementById('dias_vigencia');

            const archivoInput =
                document.getElementById('archivo');

            const preview =
                document.getElementById('preview');

            const alertaValidacion =
                document.getElementById('alertaValidacion');

            const mensajeValidacion =
                document.getElementById('mensajeValidacion');

            const formDocumento =
                document.getElementById('formDocumento');


            // =====================================================
            // FUNCIONES DE ERROR
            // =====================================================

            function mostrarError(campo, contenedor, mensaje) {

                campo.classList.add('is-invalid');

                const elemento =
                    document.getElementById(contenedor);

                if (elemento) {

                    elemento.innerHTML =
                        '<div class="invalid-feedback d-block">' +
                        mensaje +
                        '</div>';

                }

            }


            function limpiarError(campo, contenedor) {

                campo.classList.remove('is-invalid');

                const elemento =
                    document.getElementById(contenedor);

                if (elemento) {

                    elemento.innerHTML = '';

                }

            }


            // =====================================================
            // FECHA DE EMISIÓN
            // =====================================================

            fechaEmision.addEventListener('change', function () {

                limpiarError(
                    fechaEmision,
                    'error_fecha_emision'
                );


                if (fechaEmision.value) {

                    /*
                     * La fecha de vencimiento debe ser
                     * igual o posterior a la emisión.
                     */
                    fechaVencimiento.min =
                        fechaEmision.value;

                } else {

                    fechaVencimiento.removeAttribute('min');

                }

                calcularDiasVigencia();

            });


            // =====================================================
            // FECHA DE VENCIMIENTO
            // =====================================================

            fechaVencimiento.addEventListener('change', function () {

                limpiarError(
                    fechaVencimiento,
                    'error_fecha_vencimiento'
                );


                if (
                    fechaEmision.value &&
                    fechaVencimiento.value
                ) {

                    /*
                     * La fecha de vencimiento NO puede
                     * ser anterior a la fecha de emisión.
                     *
                     * La misma fecha SÍ está permitida.
                     */

                    if (
                        fechaVencimiento.value <
                        fechaEmision.value
                    ) {

                        mostrarError(
                            fechaVencimiento,
                            'error_fecha_vencimiento',
                            'La fecha de vencimiento no puede ser anterior a la fecha de emisión.'
                        );

                    }

                }

                calcularDiasVigencia();

            });


            // =====================================================
            // CALCULAR DÍAS DE VIGENCIA
            // =====================================================

            function calcularDiasVigencia() {

                if (
                    !fechaEmision.value ||
                    !fechaVencimiento.value
                ) {

                    diasVigencia.innerHTML = '';

                    alertaValidacion.classList.add('d-none');

                    return;

                }


                const emision =
                    new Date(fechaEmision.value + 'T00:00:00');

                const vencimiento =
                    new Date(fechaVencimiento.value + 'T00:00:00');

                const hoy =
                    new Date();


                /*
                 * Si la fecha de vencimiento es anterior
                 * a la emisión no se calcula la vigencia.
                 */

                if (vencimiento < emision) {

                    diasVigencia.innerHTML =
                        '<span class="badge bg-danger">' +
                        'La fecha de vencimiento no puede ser anterior a la emisión' +
                        '</span>';

                    alertaValidacion.classList.remove(
                        'd-none',
                        'alert-info',
                        'alert-success',
                        'alert-warning'
                    );

                    alertaValidacion.classList.add(
                        'alert-danger'
                    );

                    mensajeValidacion.textContent =
                        'Fechas inválidas. Corrija la fecha de vencimiento.';

                    return;

                }


                const diffTime =
                    vencimiento - hoy;

                const diffDays =
                    Math.ceil(
                        diffTime /
                        (1000 * 60 * 60 * 24)
                    );


                let badgeClass =
                    'success';

                let mensaje =
                    '';


                if (diffDays < 0) {

                    badgeClass =
                        'danger';

                    mensaje =
                        `Este documento estará VENCIDO (${Math.abs(diffDays)} días atrás)`;

                    alertaValidacion.classList.remove(
                        'd-none',
                        'alert-info',
                        'alert-success',
                        'alert-warning'
                    );

                    alertaValidacion.classList.add(
                        'alert-danger'
                    );

                } else if (diffDays <= 30) {

                    badgeClass =
                        'warning';

                    mensaje =
                        'Este documento estará próximo a vencer';

                    alertaValidacion.classList.remove(
                        'd-none',
                        'alert-info',
                        'alert-danger',
                        'alert-success'
                    );

                    alertaValidacion.classList.add(
                        'alert-warning'
                    );

                } else {

                    mensaje =
                        `Documento válido por ${diffDays} días`;

                    alertaValidacion.classList.remove(
                        'd-none',
                        'alert-info',
                        'alert-danger',
                        'alert-warning'
                    );

                    alertaValidacion.classList.add(
                        'alert-success'
                    );

                }


                diasVigencia.innerHTML =
                    `<span class="badge bg-${badgeClass}">
                Vigencia: ${diffDays} días desde hoy
            </span>`;

                mensajeValidacion.textContent =
                    mensaje;

            }


            // =====================================================
            // PREVISUALIZACIÓN DEL ARCHIVO
            // =====================================================

            archivoInput.addEventListener('change', function (e) {

                const file =
                    e.target.files[0];


                if (!file) {

                    preview.innerHTML = '';

                    return;

                }


                const fileSize =
                    file.size / 1024 / 1024;

                const fileType =
                    file.type;


                /*
                 * Validar tamaño.
                 */

                if (fileSize > 5) {

                    archivoInput.classList.add(
                        'is-invalid'
                    );

                    preview.innerHTML =
                        '<div class="invalid-feedback d-block">' +
                        'El archivo supera el tamaño máximo permitido de 5MB.' +
                        '</div>';

                    archivoInput.value = '';

                    return;

                }


                archivoInput.classList.remove(
                    'is-invalid'
                );


                /*
                 * Validar extensión.
                 */

                const extensionesPermitidas = [
                    'application/pdf',
                    'image/jpeg',
                    'image/png'
                ];


                if (
                    !extensionesPermitidas.includes(
                        fileType
                    )
                ) {

                    archivoInput.classList.add(
                        'is-invalid'
                    );

                    preview.innerHTML =
                        '<div class="invalid-feedback d-block">' +
                        'El archivo debe ser PDF, JPG o PNG.' +
                        '</div>';

                    archivoInput.value = '';

                    return;

                }


                const fileSizeTexto =
                    fileSize.toFixed(2);


                preview.innerHTML = `
            <div class="alert alert-success">
                <i class="fas fa-file"></i>
                <strong>${file.name}</strong>
                <br>
                <small>
                    Tamaño: ${fileSizeTexto} MB |
                    Tipo: ${fileType}
                </small>
            </div>
        `;

            });


            // =====================================================
            // ESTABLECER MIN CON OLD()
            // =====================================================

            if (fechaEmision.value) {

                fechaVencimiento.min =
                    fechaEmision.value;

            }


            // =====================================================
            // VALIDACIÓN ANTES DE ENVIAR
            // =====================================================

            formDocumento.addEventListener(
                'submit',
                function (e) {

                    let formularioValido =
                        true;


                    // =============================================
                    // LIMPIAR ERRORES JAVASCRIPT
                    // =============================================

                    limpiarError(
                        fechaEmision,
                        'error_fecha_emision'
                    );

                    limpiarError(
                        fechaVencimiento,
                        'error_fecha_vencimiento'
                    );


                    // =============================================
                    // FECHA DE EMISIÓN
                    // =============================================

                    if (!fechaEmision.value) {

                        mostrarError(
                            fechaEmision,
                            'error_fecha_emision',
                            'Debe seleccionar la fecha de emisión.'
                        );

                        formularioValido =
                            false;

                    }


                    // =============================================
                    // FECHA DE VENCIMIENTO
                    // =============================================

                    if (!fechaVencimiento.value) {

                        mostrarError(
                            fechaVencimiento,
                            'error_fecha_vencimiento',
                            'Debe seleccionar la fecha de vencimiento.'
                        );

                        formularioValido =
                            false;

                    }


                    // =============================================
                    // COMPARAR FECHAS
                    // =============================================

                    if (
                        fechaEmision.value &&
                        fechaVencimiento.value
                    ) {

                        if (
                            fechaVencimiento.value <
                            fechaEmision.value
                        ) {

                            mostrarError(
                                fechaVencimiento,
                                'error_fecha_vencimiento',
                                'La fecha de vencimiento no puede ser anterior a la fecha de emisión.'
                            );

                            formularioValido =
                                false;

                        }

                    }


                    // =============================================
                    // ARCHIVO
                    // =============================================

                    if (archivoInput.files.length > 0) {

                        const file =
                            archivoInput.files[0];

                        const fileSize =
                            file.size / 1024 / 1024;

                        const extensionesPermitidas = [
                            'application/pdf',
                            'image/jpeg',
                            'image/png'
                        ];


                        if (fileSize > 5) {

                            archivoInput.classList.add(
                                'is-invalid'
                            );

                            preview.innerHTML =
                                '<div class="invalid-feedback d-block">' +
                                'El archivo supera el tamaño máximo permitido de 5MB.' +
                                '</div>';

                            formularioValido =
                                false;

                        }


                        if (
                            !extensionesPermitidas.includes(
                                file.type
                            )
                        ) {

                            archivoInput.classList.add(
                                'is-invalid'
                            );

                            preview.innerHTML =
                                '<div class="invalid-feedback d-block">' +
                                'El archivo debe ser PDF, JPG o PNG.' +
                                '</div>';

                            formularioValido =
                                false;

                        }

                    }


                    // =============================================
                    // SI HAY ERROR, NO ENVIAR
                    // =============================================

                    if (!formularioValido) {

                        e.preventDefault();


                        alertaValidacion.classList.remove(
                            'd-none',
                            'alert-info',
                            'alert-success',
                            'alert-warning'
                        );

                        alertaValidacion.classList.add(
                            'alert-danger'
                        );

                        mensajeValidacion.textContent =
                            'Corrija los errores indicados en el formulario antes de registrar el documento.';


                        /*
                         * Llevar al usuario al primer campo
                         * que tenga error.
                         */

                        const primerError =
                            formDocumento.querySelector(
                                '.is-invalid'
                            );

                        if (primerError) {

                            primerError.focus();

                            primerError.scrollIntoView({
                                behavior: 'smooth',
                                block: 'center'
                            });

                        }


                        return false;

                    }


                    /*
                     * Si todo está correcto, permitir
                     * que Laravel procese el formulario.
                     */

                    return true;

                }
            );


            // =====================================================
            // QUITAR ERRORES AL CORREGIR CAMPOS
            // =====================================================

            fechaEmision.addEventListener(
                'input',
                function () {

                    limpiarError(
                        fechaEmision,
                        'error_fecha_emision'
                    );

                }
            );


            fechaVencimiento.addEventListener(
                'input',
                function () {

                    limpiarError(
                        fechaVencimiento,
                        'error_fecha_vencimiento'
                    );

                }
            );


            // =====================================================
            // CALCULAR AL CARGAR SI EXISTEN OLD()
            // =====================================================

            if (
                fechaEmision.value &&
                fechaVencimiento.value
            ) {

                calcularDiasVigencia();

            }

        });

    </script>

    <style>

        .card {
            border-radius: 10px;
        }

        .form-label {
            font-weight: 600;
        }

        .text-danger {
            font-weight: bold;
        }

        .invalid-feedback {
            font-size: 0.875rem;
        }

        .form-control.is-invalid,
        .form-select.is-invalid {
            border-color: #dc3545;
        }

        .form-control.is-invalid:focus,
        .form-select.is-invalid:focus {
            border-color: #dc3545;
            box-shadow: 0 0 0 0.25rem rgba(220, 53, 69, 0.15);
        }

    </style>

@endsection
